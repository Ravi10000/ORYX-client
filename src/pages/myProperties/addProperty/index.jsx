...
export default function AddProperty() {
  ...
  async function getPropertyType() {
    try {
      const {
        data: { data },
      } = await api.get("/property-type/read");
      // console.log(data);
  ...
  async function addProperty(data) {
    ...
    try {
      const res = await api.postForm("/property/add", {
        ...dataClone,
        status: "Added",
        url,
  ...
  return (
    <form
      ...
      className="bg-white flex flex-col shadow-2xl rounded-lg my-2 lg:m-10 pb-10"
    >
      <div className="flex flex-wrap items-center gap-5 lg:gap-10 mt-3 lg:mt-8 px-5 lg:px-10 text-lg bg-bodyBg font-semibold">
        <h2 className="lg:py-3 px-2 lg:px-10 text-lg bg-bodyBg font-semibold">
          Project 101
        </h2>

        <div className="flex lg:py-3 gap-5 lg:px-3 text-xs">
          <button className="bg-[#e0cdfb] px-2 py-1 text-[#a36df3] font-semibold border border-[#a36df3] rounded-full">
            Appointment
          </button>

          <button className="bg-[#bde4d5] px-2 py-1 text-[#47b080] font-semibold border border-[#47b080] rounded-full">
            Under Development
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 lg:gap-10 px-5 lg:px-10">
        <div className="flex-1 lg:w-[50%]">
              ...
              className={`
                            ${
                              activeTab === "basicDetails"
                                ? "text-primary underline-offset-8 decoration-2 underline"
                                : ""
                            }
                            `}
          ...
          {activeTab === "NFTsDetails" && <NFTsDetails register={register} />}
